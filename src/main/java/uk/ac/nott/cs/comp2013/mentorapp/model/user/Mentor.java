...
public class Mentor implements User, HasAvailability {
    ...
    // New method to generate available times for the mentor
    public List<String> getAvailableTimes() {
        List<String> availableTimes = new ArrayList<>();
        if (startAvailability != null && endAvailability != null) {
            LocalDateTime currentTime = startAvailability;
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
            while (!currentTime.isAfter(endAvailability)) {
                availableTimes.add(currentTime.format(formatter));
                currentTime = currentTime.plusHours(1); // Add 1 hour increment
            }
        }
